...
function Modules(){
  const {localTime}=useScene();
  const mods=[
    ['Consultation','À distance','Téléconsultation vidéo',C.blue],
    ['Consultation','En présentiel','Cabinet augmenté',C.blue],
    ['Consultation','Normale','Du premier contact au suivi',C.blue],
    ['Consultation','Dermato','Analyse d’images',C.blue],
    ['Connexion','Labo','Résultats auto-synchronisés',C.teal],
    ['Connexion','Radio','Imagerie intégrée',C.teal],
    ['Connexion','Pharmacie','E-prescription & sécurité',C.teal],
    ['Suivi','Maladies chroniques','HTA · diabète · poids — via WhatsApp',C.gold],
    ['Prévention','SilentCheck','Dépistage silencieux',C.gold],
    ['Urgence','Triage & alerte','Détection précoce',C.coral],
    ['Pilotage','Tableau de bord RH','Pour les entreprises',C.green],
  ];
  const gap=18, W=402;
  return (
    <div style={{position:'absolute',top:230,left:120,right:120,display:'flex',flexWrap:'wrap',gap:gap,justifyContent:'center'}}>
      {mods.map((m,i)=>{const p=ev(localTime,0.3+i*0.07,0.5,Easing.easeOutCubic);return(
        <div key={i} style={{width:W,opacity:p,transform:`translateY(${(1-p)*16}px)`,
          background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:14,padding:'18px 24px'}}>
